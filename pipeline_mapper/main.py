...
def main() -> None:
    ...
    ml_reg_results = regression_grid_search(
        X_features,
        lente_vals,
    )

    ml_clf_results = classification_grid_search(
        X_features,
        cat_vals,
    )
    ...
    print(f"  Mejor regresor           : "f"{best_reg['modelo']} ({best_reg['best_score']:.3f})")
    print(f"  Mejor clasificador       : "f"{best_clf['modelo']} ({best_clf['best_score']:.3f})")
